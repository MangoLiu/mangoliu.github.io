...
【LTR&时效性联合项目】立项说明
【背景】：
在相当比例的query中其实有时效性需求，用户期望新的结果，我们需要从通用的角度对每个用户的query做时效性的量化识别。
黄金价格，时效性：1.0
北大考研，时效性：0.8
医疗设备制度，时效性：0.6
对有时效性query的情况下，我们需要对相关的、较新的结果进行提权，以满足用户对结果页面的时效性需求。
【项目目标】：
       一个soft的、通用的泛时效性解决方案，解决线上泛时效性query的调权问题。
【项目解决方案】：
        整体解决泛时效性的策略框架：
...
时效性的标注
多目标优化
页面因子特征和query时效性度量特征的融合生效

【人力】：

       时效性：国军、樊骏（挖掘及其他时效性策略、特征的产出）
...
注：后续，query的时效性识别策略框架及特征、语义特征及产出、时效性的AC-LTR模型调权分开发帖。
============
时效性分类问题：
对于文本 query 给出其从语义角度有时效性需求的概率（见立项说明）。

 
项目预期产出：
时效性语义扩展数据（用于扩大召回）：
现有时效性策略是对时效性 query 集合建立词典，并判断命中，本项目第一步为产出时效性query 数据，这部分数据可以由开发训练中的模型在现有 query 集合上预测得到，并送 PM进行质量评估。
1. 时效性Term 数据（对现有时效性 Term 进行扩展）
2. 时效性 Query 数据 （对已有时效性 Query 词典进行扩展）

时效性语义模型：
一个query 具有时效性需求概率的模型，用于线上query 分类并针对不同的query 激活相关时效性策略。


项目规划：
完善评测标准和评测数据集（随项目进度累计）
使用挖掘数据和 部分标注数据训练 DNN模型并使用模型在 query 集合上预测扩大 query 词典（需要进行训练数据比例调整，越需要2周）
使用自然文本数据初始化DNN模型，用挖掘数据训练并用部分标注数据调参 （需要两周左右）
使用 DBN和 RBM 构造时效性判别模型（如果DNN 模型效果需要得到进一步提升，约需要3周时间）


评测标准：
评价测试集合：
测试集合选择为 随机抽取query 数据的人工标注数据，初步估计需要 5000左右的标注数据量用于测试（测试数据可以随项目推进进行累积）
测试集合只能用于评测目的，不允许使用测试集合进行训练，模型筛选和参数调优。


评价测试标准（全部都在测试集合进行计算）：
对于连续概率输出的模型，评测标准为 ROC 曲线下面积 (AUC)
对于离散 0,1 值分类器或者挖掘策略，评价标准为 模型或策略的 F1 score ( 召回率和 精度的几何平均)。
对比连续模型和离散模型使用连续模型的F1 最优阀值（在 validation 集合 ）产生的 F1 score。

 
训练用数据：
现有策略挖掘出的时效性 query ( 现有100万左右，实际训练需要可能为 1亿左右，需要进行更大规模的 数据 dump）
标注数据 （3千左右的标注数据），标注数据用于部分模型的直接训练和模型参数以及超参的调优，与测试数据分离
自然语言数据，query log 的query 文本数据（包含query 频率），和百科文本数据（用于初始化语言模型参数，由于标注数据稀缺需要使用半监督方法）
